#!/usr/bin/env python3
"""
🌙 Moon Dev's Momentum Strategy Multi-Coin Backtester
Tests momentum strategies across all available coins
"""
...
OUTPUT_DIR = Path("/home/titus/moon-dev-ai-agents/src/data/momentum_backtest")
RESULTS_CSV = OUTPUT_DIR / "momentum_backtest_results.csv"

# Only test these strategies
MOMENTUM_STRATEGIES = [
    "T01_VolumetricSupertrend_OPT_v4_7.2pct.py",
    "T05_VolumetricBreakout_OPT_v4_51.4pct.py"
]
...
def get_strategy_files():
    """Get momentum strategy files only"""
    return [STRATEGIES_DIR / name for name in MOMENTUM_STRATEGIES]
...
def extract_coin_name(filepath: Path) -> str:
    """Extract coin name from filepath"""
...
def run_backtest(strategy_file: Path, data_file: Path) -> dict:
    ...
    try:
        ...
        # Pattern: Lines with os.path.join that mention 'rbi' or '.csv'
        ...
        if return_pct is not None:
            diff = return_pct - buy_hold if buy_hold is not None else return_pct
            color = "green" if diff > 0 else "yellow"
            cprint(f"  ✅ {coin_name}: {return_pct:>7.2f}% (B&H: {buy_hold:>7.2f}%, Diff: {diff:>+7.2f}%)", color)
        else:
            cprint(f"  ⚠️  {coin_name}: Could not parse results", "yellow")
        ...
        return {
            ...
            'diff': return_pct - buy_hold if (return_pct and buy_hold) else None,
            'sharpe_ratio': sharpe,
...
def main():
    """Run batch backtests"""
    cprint("\n" + "=" * 70, "white")
    cprint("🌙 Moon Dev's Momentum Strategy Backtester", "yellow", attrs=["bold"])
    cprint("=" * 70 + "\n", "white")
    ...
    cprint(f"📊 Found {len(strategy_files)} momentum strategies", "cyan")
    ...
    # Summary
    cprint("\n" + "=" * 120, "white")
    cprint("✨ Momentum Strategy Backtesting Complete!", "green", attrs=["bold"])
    cprint("=" * 120, "white")
    ...
    if successful > 0:
        ...
        df_success['buy_hold_pct'] = pd.to_numeric(df_success['buy_hold_pct'], errors='coerce')
        df_success['diff'] = pd.to_numeric(df_success['diff'], errors='coerce')
        
        # Top by absolute return
        top_10 = df_success.nlargest(10, 'return_pct')
        
        cprint("\n🏆 Top 10 Performers (by Return):", "yellow", attrs=["bold"])
        cprint("=" * 120, "white")
        cprint(
            f"{'#':<3} {'Strategy':<25} {'Coin':<6} {'Return':<9} {'B&H':<9} "
            f"{'Diff':<9} {'Drawdown':<10} {'Trades':<8} {'Win Rate':<18} {'Sharpe':<8}",
            "cyan", attrs=["bold"]
        )
        cprint("-" * 120, "white")
        ...
        for rank, (idx, row) in enumerate(top_10.iterrows(), 1):
            ...
            diff = row['diff'] if pd.notna(row['diff']) else 0
            ...
            print(
                f"{rank:<3} {row['strategy']:<25} {row['coin']:<6} "
        ...
        # Best vs B&H
        beat_bh = df_success[df_success['diff'] > 0]
        if len(beat_bh) > 0:
            cprint(f"\n🎯 Strategies Beating Buy & Hold: {len(beat_bh)}/{successful}", "green", attrs=["bold"])
            top_diff = beat_bh.nlargest(5, 'diff')
            cprint("-" * 120, "white")
            for idx, row in top_diff.iterrows():
                cprint(
                    f"  {row['strategy']:<25} on {row['coin']:<6}: "
                    f"{row['return_pct']:>7.2f}% vs B&H {row['buy_hold_pct']:>7.2f}% "
                    f"(+{row['diff']:>6.2f}%)",
                    "green"
                )
